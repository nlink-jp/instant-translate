...
- **賢い言語ルーティング**: 入力言語は自動認識。出力は既定でシステムの言語へ。
  *自分の言語* で入力した場合は、出力を設定した **二次言語**（例: 英語）へ自動で
  ...
  認識が拮抗する場合（漢字だけの文は日本語とも中国語とも読める、など）は
  **自分の言語（システム言語 + 二次言語）を優先**して自動選択し、決めた言語を
  翻訳エンジンにも明示するので、macOS の「どの言語ですか？」選択ダイアログに
  さえぎられません。
- **入力言語の固定**: パネル左のピッカー（Auto + 言語）で入力言語をピン留めできます。
  固定中は認識をスキップして常にその言語として翻訳します — 貼り付ける文章の言語が
  分かっているときに（アプリ再起動で Auto に戻ります）。
